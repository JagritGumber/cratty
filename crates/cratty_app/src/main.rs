...
mod clipboard; mod clipboard_ops; mod home; mod layers; mod message;
...
impl Cratty {
    fn new() -> (Self, Task<Message>) {
        ...
        let qk = if config.quake_mode.enabled {
            quake::QuakeHotkey::register(&config.quake_mode.hotkey)
        } else { None };
        let mut app = Self {
            config, metrics,
            ...
            pending_backends: vec![], viewport_w: 1200.0, _quake: qk,
        };
        app.restore_layout();
        (app, Task::none())
    }
    ...
    fn view(&self) -> Element<'_, Message> {
        ...
        if let Some(popup_layer) = self.view_popups() {
            view_layers.push(popup_layer);
        }
    ...
    fn view_popups(&self) -> Option<Element<'_, Message>> { self.menu_overlay() }
    fn theme(&self) -> Theme { style::cratty_theme() }
    fn subscription(&self) -> Subscription<Message> {
        Subscription::batch([app_keys::subscription(), quake::subscription()])
    }
}
